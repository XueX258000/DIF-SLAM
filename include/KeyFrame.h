...
protected:
    ...
    // DIF-SLAM v2.0: per-KeyFrame forbid mask for map writing (M_forbid_map == M_D, CV_8U).
    // This is a snapshot captured in Tracking when the KeyFrame is created and is used by LocalMapping
    // to mitigate segmentation lag/missing (P-002) without accessing Tracking thread internals.
    cv::Mat mDIFForbidMask;
...
public:
    ...
    // DIF-SLAM v2.0: forbid mask for map writing (M_forbid_map == M_D). Optional; empty if not available.
    void SetDIFForbidMask(const cv::Mat& mask)
    {
        if(mask.empty())
        {
            mDIFForbidMask.release();
            return;
        }
        if(mask.type() == CV_8U)
            mDIFForbidMask = mask;
        else
            mask.convertTo(mDIFForbidMask, CV_8U);
    }

    const cv::Mat& GetDIFForbidMask() const { return mDIFForbidMask; }
