...
class MapPoint
{
    ...
    void serialize(Archive & ar, const unsigned int version)
    {
        ...
        if(version >= 2)
        {
            ar & mnDIFCreatedFrameId;
            ar & mbDIFMapLockAtCreate;
            ar & mfDIFWLockAtCreate;
        }
        else
        {
            mnDIFCreatedFrameId = -1;
            mbDIFMapLockAtCreate = false;
            mfDIFWLockAtCreate = 1.0f;
        }
    }
...
public:
    ...
    // DIF (Dynamic Instance Filtering): global instance track id, -1 for background/unknown.
    int mnInstanceId = -1;
    // DIF-SLAM v2.0: bookkeeping for lock-weighted mapping and post-hoc debugging.
    int mnDIFCreatedFrameId = -1;
    bool mbDIFMapLockAtCreate = false;
    float mfDIFWLockAtCreate = 1.0f;
...
BOOST_CLASS_VERSION(ORB_SLAM3::MapPoint, 2)
