...
static inline double DIFInfoWeight(const MapPoint* pMP)
{
    if(!pMP)
        return 1.0;
    const double w = static_cast<double>(pMP->mfDIFWLockAtCreate);
    if(!std::isfinite(w))
        return 1.0;
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0)
            {
                ...
                e->setInformation(Eigen::Matrix2d::Identity() * (static_cast<double>(invSigma2) * DIFInfoWeight(pMP)));
            ...
            else if(leftIndex != -1 && pKF->mvuRight[leftIndex] >= 0) //Stereo observation
            {
                ...
                e->setMeasurement(obs);
                const float &invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];
                Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() * (static_cast<double>(invSigma2) * DIFInfoWeight(pMP));
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                else if(leftIndex != -1 && pKFi->mvuRight[leftIndex] >= 0) // stereo observation
                {
                    ...
                    e->setInformation(Eigen::Matrix3d::Identity() * (static_cast<double>(invSigma2) * DIFInfoWeight(pMP)));
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mvuRight[get<0>(mit->second)]<0) //Monocular
            {
                ...
                e->setMeasurement(obs);
                const float &invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];
                e->setInformation(Eigen::Matrix2d::Identity() * (static_cast<double>(invSigma2) * DIFInfoWeight(pMPi)));
            ...
            else // RGBD or Stereo
            {
                ...
                e->setMeasurement(obs);
                const float &invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];
                Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() * (static_cast<double>(invSigma2) * DIFInfoWeight(pMPi));
                e->setInformation(Info);
