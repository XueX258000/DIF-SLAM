...
void LocalMapping::EnqueueDIFPruneTrackIds(const std::vector<int>& ids)
{
    if(ids.empty())
        return;
    std::lock_guard<std::mutex> lock(mMutexDIFPrune);
    for(const int id : ids)
    {
        if(id >= 0)
            mDIFPendingPruneTrackIds.insert(id);
    }
}
...
void LocalMapping::ProcessDIFPendingPrunes()
{
    if(!mpTracker || !mpTracker->IsDIFEnabled())
        return;

    std::unordered_set<int> pending;
    {
        std::lock_guard<std::mutex> lock(mMutexDIFPrune);
        if(mDIFPendingPruneTrackIds.empty())
            return;
        pending.swap(mDIFPendingPruneTrackIds);
    }
    ...
    const std::vector<MapPoint*> points = pMap->GetAllMapPoints();
    int pruned = 0;
    ...
    std::cout << "[DIF-PRUNE-LM] tracks=" << pending.size()
              << " pruned_mps=" << pruned
              << " map_id=" << pMap->GetId()
              << std::endl;
}
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    int dif_bind_total = 0;
    int dif_bind_assigned = 0;
    int dif_bind_conflict = 0;
    int dif_bind_suppressed = 0;
    int dif_bind_forbid_mask = 0;
    int dif_bind_seg_missing = 0;

    auto in_dif_mask = [](const cv::Mat& mask, const cv::Point2f& pt) -> bool {
    ...
    const bool dif_enabled = (mpTracker && mpTracker->IsDIFEnabled());
    const bool dif_filter = (dif_enabled && mpTracker->IsDIFFilterEnabled());
    const bool dif_kf1_has_map =
        (dif_enabled ? mpTracker->HasDIFSegMappingForFrameId(static_cast<int>(mpCurrentKeyFrame->mnFrameId)) : false);
    const cv::Mat& dif_kf1_forbid = mpCurrentKeyFrame->GetDIFForbidMask();
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        const bool dif_kf2_has_map =
            (dif_enabled ? mpTracker->HasDIFSegMappingForFrameId(static_cast<int>(pKF2->mnFrameId)) : false);
        const cv::Mat& dif_kf2_forbid = pKF2->GetDIFForbidMask();
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            int gid = -1;
            bool created_missing_map = false;
            if(dif_enabled)
            {
                dif_bind_total++;
                int gid1 = -1;
                ...
                {
                    pt1 = kp1.pt;
                    if(idx1 >= 0 && idx1 < static_cast<int>(mpCurrentKeyFrame->mvKeys.size()))
                        pt1 = mpCurrentKeyFrame->mvKeys[static_cast<size_t>(idx1)].pt;
                    has_pt1 = true;
                    if(dif_kf1_has_map)
                        gid1 = mpTracker->GetDIFGlobalTrackIdAtFrameId(static_cast<int>(mpCurrentKeyFrame->mnFrameId), pt1);
                }

                if(!bRight2)
                {
                    pt2 = kp2.pt;
                    if(idx2 >= 0 && idx2 < static_cast<int>(pKF2->mvKeys.size()))
                        pt2 = pKF2->mvKeys[static_cast<size_t>(idx2)].pt;
                    has_pt2 = true;
                    if(dif_kf2_has_map)
                        gid2 = mpTracker->GetDIFGlobalTrackIdAtFrameId(static_cast<int>(pKF2->mnFrameId), pt2);
                }
                ...
                if(dif_filter)
                {
                    if(gid >= 0 && (mpTracker->IsDIFTrackSuppressed(gid) || mpTracker->IsDIFTrackForbiddenForMap(gid)))
                    {
                        dif_bind_suppressed++;
                        continue;
                    }

                    // If we fail to bind a global_id, fall back to per-KeyFrame M_forbid_map snapshots.
                    // This mitigates segmentation lag/missing and segmentation holes at boundaries (P-002).
                    if(gid < 0)
                    {
                        bool forbid = false;
                        if(has_pt1 && in_dif_mask(dif_kf1_forbid, pt1))
                            forbid = true;
                        if(!forbid && has_pt2 && in_dif_mask(dif_kf2_forbid, pt2))
                            forbid = true;
                        if(forbid)
                        {
                            dif_bind_forbid_mask++;
                            continue;
                        }
                    }
                    created_missing_map = (!dif_kf1_has_map || !dif_kf2_has_map);
                }
            }
            ...
            if(dif_enabled)
            {
                pMP->mnInstanceId = gid;
                pMP->mnDIFCreatedFrameId = static_cast<int>(mpCurrentKeyFrame->mnFrameId);
                if(gid >= 0 && mpTracker)
                {
                    bool map_lock = false;
                    float w_lock = 1.0f;
                    if(mpTracker->GetDIFTrackLockInfo(gid, map_lock, w_lock))
                    {
                        pMP->mbDIFMapLockAtCreate = map_lock;
                        pMP->mfDIFWLockAtCreate = w_lock;
                    }
                }
                if(gid >= 0)
                    dif_bind_assigned++;
                if(dif_filter && created_missing_map)
                    dif_bind_seg_missing++;
            }
            if (bPointStereo)
                countStereo++;
    ...
    if(mpTracker && mpTracker->IsDIFEnabled() && dif_bind_total > 0)
    {
        std::cout << "[DIF-BIND] LocalMapping created MPs=" << dif_bind_total
                  << " assigned=" << dif_bind_assigned
                  << " conflict=" << dif_bind_conflict
                  << " suppressed=" << dif_bind_suppressed
                  << " forbid_mask=" << dif_bind_forbid_mask
                  << " missing_map_created=" << dif_bind_seg_missing
                  << " kf=" << mpCurrentKeyFrame->mnId
                  << " frame_id=" << mpCurrentKeyFrame->mnFrameId
                  << std::endl;
    }
}
