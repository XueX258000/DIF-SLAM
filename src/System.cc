    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //if(false) // TODO
    {
        ...
        mViewerThreadId = mptViewer->get_id();
    ...
    // Start DIF SegmentationWorker thread after all other threads are initialized
    // This avoids multi-thread race condition during system startup
    mpTracker->StartDIFSegmentationWorker();
    mpTracker->StartDenseMappingWorker();
...
void System::Shutdown()
{
    ...
    if(mpTracker)
        mpTracker->StopDenseMappingWorker();
    if(mpViewer)
    {
        mpViewer->RequestFinish();
        const bool wait_viewer = mptViewer && (std::this_thread::get_id() != mViewerThreadId);
        if(wait_viewer)
        {
...
bool System::SaveDensePointCloud(const string &filename, bool use_latest_pose)
{
    if(!mpTracker)
        return false;
    ...
    if(pMap)
    {
        // Ensure a consistent snapshot of keyframe poses (avoid exporting while GBA/loop updates are in-flight).
        std::unique_lock<std::mutex> lock(pMap->mMutexMapUpdate);
        return mpTracker->SaveDensePointCloud(filename, use_latest_pose);
    }
    return mpTracker->SaveDensePointCloud(filename, use_latest_pose);
}
